...
# function
nt<-function (ftn, x0, tol = 1e-09, max.iter = 30) {
  x <- x0
  fx <- ftn(x)
  iter <- 0
...
for (i in 1:iter) {
  ...
  # 1) bootstrap : 2sls
  ratio_est2<-foreach (j = 1:boot) %dopar% {
    ...
    # 1)ratio of coef_se_bootstrap
    denom<-lm(A~Z)
    numer<-glm(Y~Z,family = 'poisson')
    
    numer$coefficients[2]/denom$coefficients[2]
  ...
  g_coef2<-foreach (j = 1:boot) %dopar% {
    ...
    gestimation_ftn(Z=Z,A=A,U=U,Y=Y)
    
  }
  stopImplicitCluster()
  
  # list -> vector
  ratio_est2<-unlist(ratio_est2);g_coef2<-unlist(g_coef2)
